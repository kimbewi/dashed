def public_deficiency_chart():    
    ...
    # Calculate year-over-year deficiency ratios for Public + SUCs/LUCs
    ...
    for i in range(1, len(sorted_years)):
        ...
        public_current = enrollment_totals[current_year]["Public + SUCs/LUCs"]
        public_previous = enrollment_totals[previous_year]["Public + SUCs/LUCs"]

        public_ratio = public_current / public_previous
        public_ratio = min(1.00, public_ratio)
        public_decrease = 1.00 - public_ratio

        data.append({
            "Year": f"{previous_year}-{current_year}",
            "Deficiency Ratio": public_decrease,
            "Previous Year Enrollees": public_previous,
            "Current Year Enrollees": public_current
        })

    # Convert to DataFrame
    df_plot = pd.DataFrame(data)

    # Convert deficiency ratio to percentage
    ...
    fig = px.line(
        ...
        x="Year",
        y="Deficiency Ratio",
        title="Enrollment Deficiency Ratio for Public + SUCs/LUCs Schools Over the Years",
        markers=True,
        labels={"Deficiency Ratio": "1 - Capped Ratio"},
    )
    ...
    fig.update_traces(
        hovertemplate=(
            ...
            "Previous Year Enrollees: %{customdata[0]:,.0f}"
